...
${topic('ti-crosshair', 'Sessões de farme (por DG)', `
  ...
  ${li([
    ...
    '<strong>Encerrou e os drops voltaram? Ele retoma a mesma sessão.</strong> Por padrão ele fecha após 5min sem drop (ajustável), fechando <strong>no horário do último drop</strong> — o tempo parado nunca entra na conta. E se você voltar a dropar dentro de 3× esse limite, ele continua a sessão anterior em vez de abrir outra, então um farme longo com pausas não vira dez sessões picadas. Só abre uma nova se os itens indicarem que você trocou de DG.',
