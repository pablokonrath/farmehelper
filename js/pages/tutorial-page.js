...
${topic('ti-coins', 'Cálculo de farme (preços & itens rastreados)', `
  ...
  ${li([
    ...
    '<strong>Itens que você não vende</strong> (ficha de evento, moeda, material de consumo) têm uma lista própria nesta página: eles continuam aparecendo no log e nos itens da sessão, mas <strong>não valem Alz</strong> em conta nenhuma. Item que nunca vira dinheiro contando como farme infla o total do dia e faz você escolher a DG errada — e isso acontece sozinho, porque basta o item ter preço de referência da comunidade. A lista casa por trecho do nome ("fragmento" pega todas as variações) e vale pra sempre, inclusive no histórico: nada é reavaliado retroativamente quando um evento acaba.',
...
${topic('ti-crosshair', 'Sessões de farme (por DG)', `
  ...
  ${li([
    ...
    '<strong>A sessão abre no primeiro drop</strong>, sem esperar nada — e pode abrir <strong>sem DG</strong>, só segurando o tempo. Sessão sem DG fica fora de todas as médias até você escolher qual era, então ela nunca suja o número de outra DG. Se cair um item que só existe numa DG, o FarmHub marca sozinho.',
