...
${itemsWithoutPrice.length ? `<div class="notice"><i class="ti ti-alert-circle" style="flex-shrink:0;margin-top:1px"></i><div><strong>${itemsWithoutPrice.length} itens sem preço</strong> no DropList.<div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:6px">${itemsWithoutPrice.slice(0, 6).map(i => `<span style="cursor:pointer;background:var(--warn-bg);border:1px solid var(--warn-border);border-radius:20px;padding:2px 9px;font-size:11px;color:var(--warn)" onclick="document.getElementById('cN').value='${i.name}'">${i.name} (${i.qty}×)</span>`).join('')}</div></div></div>` : ''}
