...
const nickCell = nick => `<span style="display:inline-flex;align-items:center;gap:6px">${nick}<button title="Copiar nick pra chamar no chat do jogo" style="background:transparent;border:none;color:var(--acc);cursor:pointer;font-size:13px;padding:0;display:flex" onclick="copyNick('${nick}')"><i class="ti ti-copy"></i></button></span>`;
...
<div class="card">
  ...
  ${sent.map(o => `<tr>
    <td>${formatDateTimeBR(o.ts)}</td>
    <td>${o.itemName}</td>
    <td>${o.status === 'accepted' ? nickCell(o.sellerUsername) : o.sellerUsername}</td>
    <td style="font-weight:700;color:var(--gold)" title="${o.offerPrice.toLocaleString('pt-BR')} Alz">${formatAlzGamer(o.offerPrice)}</td>
    <td>${offerStatusBadge(o.status)}${o.status !== 'pending' && !o.buyerSeen ? ' <span class="badge badge-acc">Nova</span>' : ''}</td>
  </tr>${o.status === 'accepted' ? `<tr><td colspan="5" style="background:var(--ok-bg);padding:8px 16px;font-size:12px;color:var(--ok)"><i class="ti ti-shield-check"></i> ${SAFE_TRADE_HINT} Chame <strong>${o.sellerUsername}</strong> no jogo.</td></tr>` : ''}`).join('')}
  </tbody></table>`}
</div>`;
