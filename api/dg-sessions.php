...
if ($method === 'GET') {
  try {
    ...
    // Tabela ainda não criada (migração pendente) — devolve vazio pra não quebrar o carregamento;
    // o cliente cai no blob antigo (app_settings.dgSessions) até a migração rodar.
    json_response([]);
  }
...
if ($method === 'PUT') {
  ...
  $stmt = $db->prepare('INSERT INTO dg_sessions
